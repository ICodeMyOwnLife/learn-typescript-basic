...
run(
  ...
  log => {
    ...
    function bind<TThis, TFirst, TArgs extends any[], TResult>(
      f: (first: TFirst, ...args: TArgs) => TResult,
      thisArg: TThis,
      first: TFirst
    ): (...args: TArgs) => TResult {
    ...
    const obj = {
      a: 1,
      add(n1: number, n2: number, s: string) {
        return (this && this.a + n1 + n2) + s;
      }
    };
...
function run(
  name: string,
  executeFn: (log: (...args: any[]) => void) => any,
  isRun: boolean = false
) {
